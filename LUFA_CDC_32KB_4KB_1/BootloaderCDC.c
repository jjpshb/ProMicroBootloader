 ...
 */
//static uint16_t MagicBootKey ATTR_NO_INIT;

#define TIMEOUT     40
uint8_t timeout;

static bool ApplicationValid ATTR_NO_INIT;
...
const uint16_t BOOT_KEY_VALUE = 0x77A4;
static uint16_t BootKey ATTR_NO_INIT;
...
void Application_Jump_Check(void)
{
	...
	uint16_t bootKeyVal = BootKey;
	BootKey = 0;
  
	bool JumpToApplication = false;

	if ( (MCUSR & (1<<EXTRF)) && (bootKeyVal != BOOT_KEY_VALUE) )
  {
    MCUSR &= ~(1 << EXTRF);
    
		BootKey = BOOT_KEY_VALUE;
  ...
  else if ((MCUSR & (1 << WDRF)))
  {
	...
	ApplicationValid = (pgm_read_word_near(0) != 0xFFFF);
